...
class GameDataEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = MlbGumboSDK::test($seed, null);
        $seen = iterator_to_array($base->GameData(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = MlbGumboConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = MlbGumboSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->GameData(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MLB_GUMBO_TEST_GAME_DATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function game_data_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MLB_GUMBO_TEST_GAME_DATA_ENTID");
