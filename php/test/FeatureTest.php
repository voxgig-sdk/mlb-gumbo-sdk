...
class FtCtrl extends MlbGumboControl
{
...
class FtClient
{
    ...
    private MlbGumboUtility $utility;

    public function __construct(MlbGumboUtility $utility, array $options)
    {
    ...
    public function get_utility(): MlbGumboUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MlbGumboContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MlbGumboContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MlbGumboContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MlbGumboConfig::make_feature($name);
    ...
    private static function buildUrl(MlbGumboSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MlbGumboError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MlbGumboResponse($fetched);
        }
    ...
    private function fail(MlbGumboContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MlbGumboContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
