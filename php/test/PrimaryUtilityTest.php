...
// MlbGumbo SDK primary utility test
...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        MlbGumboSDK $client,
        MlbGumboUtility $utility
    ): MlbGumboContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new MlbGumboContext($ctxmap, null);
    ...
    private static function fixctx(MlbGumboContext $ctx, MlbGumboSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?MlbGumboError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MlbGumboError($code, $msg);
    }
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new MlbGumboBaseFeature();
    ...
    {
        $hook_client = MlbGumboSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends MlbGumboBaseFeature {
            public $hook_fn = null;
            public function TestHook(MlbGumboContext $ctx): void
            {
    ...
    {
        $init_client = MlbGumboSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends MlbGumboBaseFeature {
            public $init_fn = null;
    ...
    {
        $blocked_client = new MlbGumboSDK([
            'system' => [
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new MlbGumboSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = MlbGumboSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = MlbGumboSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof MlbGumboError) ? $result->err->msg : (string)$result->err;
