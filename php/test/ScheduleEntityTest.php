...
class ScheduleEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set MLB_GUMBO_TEST_SCHEDULE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function schedule_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("MLB_GUMBO_TEST_SCHEDULE_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "MLB_GUMBO_TEST_SCHEDULE_ENTID" => $idmap,
        "MLB_GUMBO_TEST_LIVE" => "FALSE",
        "MLB_GUMBO_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["MLB_GUMBO_TEST_SCHEDULE_ENTID"]);
