...
class MlbGumboSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = MlbGumboHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = MlbGumboHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!MlbGumboFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, MlbGumboFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return MlbGumboUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = MlbGumboHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = MlbGumboHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = MlbGumboHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new MlbGumboError($op . "_allow",
                "MlbGumboSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = MlbGumboHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new MlbGumboError("graphql_error",
                "MlbGumboSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GameData($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_game_data === null) {
                $this->_game_data = new GameDataEntity($this, null);
            }
            return $this->_game_data;
        }
        return new GameDataEntity($this, $data);
    }
