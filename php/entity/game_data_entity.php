...
class GameDataEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GameDataEntity($this->_client, $opts);
    }

    /**
     * @param GameData|array $args GameData data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MlbGumboHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GameData|array The current GameData data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GameData fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MlbGumboHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GameData fields).
     */
    ...
    /**
     * Load a single GameData.
     *
     * @param GameDataLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GameDataLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GameData|array The loaded GameData as an assoc-array at the
     *   SDK boundary; throws MlbGumboError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MlbGumboHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List GameData items matching the given filter.
     *
     * @param GameDataListMatch|array|null $reqmatch Match filter (any subset
     *   of GameData fields) as an assoc-array; GameDataListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GameData[]|array A list of GameData items as assoc-arrays at
     *   the SDK boundary; throws MlbGumboError on failure (item-5 convention).
     */
