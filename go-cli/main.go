// Command MlbGumbo-cli is a CLI + REPL for the MlbGumbo SDK,
...
const prompt = "mlb-gumbo"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "game_data player schedule team"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MLB_GUMBO_APIKEY (api key), MLB_GUMBO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MLB_GUMBO_APIKEY carries the API key and
	// MLB_GUMBO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MLB_GUMBO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MLB_GUMBO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMlbGumboSDK(opts)
