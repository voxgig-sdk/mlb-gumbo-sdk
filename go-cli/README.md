...
boru-driven command-line client **and** interactive REPL for the MlbGumbo
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mlb-gumbo-cli)
make build
...
./mlb-gumbo-cli list game_data
./mlb-gumbo-cli load 1 game_data            # {id:1} shorthand
./mlb-gumbo-cli load '{id:1}' game_data       # explicit match map
./mlb-gumbo-cli list player

# 5. Override the API base URL for a single call
MLB_GUMBO_BASE=https://api.example.com ./mlb-gumbo-cli list game_data

# 6. No arguments -> interactive REPL
./mlb-gumbo-cli
mlb-gumbo> list game_data
mlb-gumbo> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mlb-gumbo-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mlb-gumbo-cli list game_data
   ```
...
```sh
./mlb-gumbo-cli load 1 game_data          # scalar shorthand for {id:1}
./mlb-gumbo-cli load '{id:1}' game_data     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `mlb-gumbo>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `game_data`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mlb-gumbo-cli`. |
...
(`./mlb-gumbo-cli <expr>`) and interactively (the REPL), and expressions compose the
