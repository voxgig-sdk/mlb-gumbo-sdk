...
local GameDataEntity = {}
GameDataEntity.__index = GameDataEntity


function GameDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GameDataEntity)
  self._name = "game_data"
  self._client = client
...
function GameDataEntity:get_name()
  return self._name
end


function GameDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GameDataEntity.new(self._client, opts)
end
...
function GameDataEntity:mark_deleted()
  self._deleted = true
end


function GameDataEntity:deleted()
  return true == self._deleted
end


function GameDataEntity:data_set(args)
  if args ~= nil then
...
function GameDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GameDataEntity:match_set(args)
  if args ~= nil then
...
function GameDataEntity:match_get()
...
function GameDataEntity:stream(action, args, callopts)
...
---@param reqmatch GameDataLoadMatch
---@param ctrl? table
---@return GameData
---@return string? err
function GameDataEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch GameDataListMatch
---@param ctrl? table
---@return GameData[]
---@return string? err
function GameDataEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function GameDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
