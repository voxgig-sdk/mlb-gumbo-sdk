...
The Lua SDK for the MlbGumbo API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:GameData(nil):list(nil, nil)
...
```lua
local result, err = client:GameData(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:MlbGumbo(nil):load(
  { id = "test01" }, nil
)
