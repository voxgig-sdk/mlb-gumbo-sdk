...
module MlbGumboUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = MlbGumboResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MlbGumboResponse.new(fetched)
    else
