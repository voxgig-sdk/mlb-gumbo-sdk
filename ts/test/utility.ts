...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.MLB_GUMBO_TEST_LIVE ||
    'TRUE' === process.env.MLB_GUMBO_TEST_OVERRIDE
  ) {
  ...
  m.MLB_GUMBO_TEST_EXPLAIN = process.env.MLB_GUMBO_TEST_EXPLAIN || m.MLB_GUMBO_TEST_EXPLAIN
