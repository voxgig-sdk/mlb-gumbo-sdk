...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.MLBGUMBO_TEST_LIVE ||
    'TRUE' === process.env.MLBGUMBO_TEST_OVERRIDE
  ) {
  ...
  m.MLBGUMBO_TEST_EXPLAIN = process.env.MLBGUMBO_TEST_EXPLAIN || m.MLBGUMBO_TEST_EXPLAIN
