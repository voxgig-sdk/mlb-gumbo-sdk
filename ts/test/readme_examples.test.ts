...
//     (\`new MlbGumboSDK(...)\` / \`MlbGumboSDK.test(...)\` become
//     \`MlbGumboSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"game_data":{"test01":{"id":"test01"}},"player":{"test01":{"id":"test01"}},"schedule":{"test01":{"id":"test01"}},"team":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new MlbGumboSDK(...)\` and any
// \`MlbGumboSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MlbGumboSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MlbGumboSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
