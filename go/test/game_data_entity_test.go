...
func TestGameDataEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.GameData(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.GameData(nil).Stream("list", nil, nil) {
...
func game_dataBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MLB_GUMBO_TEST_GAME_DATA_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MLB_GUMBO_TEST_GAME_DATA_ENTID"])
