...
class GameDataEntity
  ...
  def make
    opts = @_entopts.dup
    GameDataEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MlbGumboHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GameData, Hash] the current GameData data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MlbGumboHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GameData fields)
  def match_get
  ...
  #
  # @param reqmatch [GameDataLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GameData.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GameData, Hash] the loaded GameData; raises MlbGumboError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = MlbGumboHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [GameDataListMatch, Hash, nil] match filter (any subset of
  #   GameData fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GameData>, Array] the matching GameData items; raises MlbGumboError on failure
